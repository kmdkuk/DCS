...
/*
  Function:   predator_isUnknown(int world[16][16], predator_position self, int dir)
  Description:  dirの方向に未探索のものがあるか，また，壁が付近にないか判定して1,0を返す．
*/
...
/*
  Function:   int predator_isVisible(predator_position it)
  Description:  渡された引数itが見えているものかどうか判断
                初期値-10,-10のままであれば見えてないと判断して0を返す．
*/
...
/*
  Function:predator_isContact(predator_position predator, predator_position prey)
  Description:引数に与えられたpredatorとpreyがくっついているかどうか判断する関数
*/
...
/*
  Function:predator_isOtherExist(int self_id, predator_position prey, int dir)
  Description:preyの側近に他のpredatorが接着しているかどうか判断する関数
*/
int predator_isOtherExist(int self_id, predator_position prey, int dir)
{
    ...
    {
    case 'u':
        if (predator_map[self_id][prey.x - 1][prey.y] > 0 && predator_map[self_id][prey.x - 1][prey.y] != self_id + 1)
            return 1;
        return 0;
        break;
    case 'r':
        if (predator_map[self_id][prey.x][prey.y + 1] > 0 && predator_map[self_id][prey.x][prey.y + 1] != self_id + 1)
            return 1;
        return 0;
        break;
    case 'd':
...
/*
  Function:predator_isExist(int self_id, predator_position prey, int dir)
  Description:preyの側近にpredatorが接着しているかどうか判断する関数
*/
int predator_isExist(int self_id, predator_position prey, int dir)
{
    ...
    {
    case 'u':
        if (predator_map[self_id][prey.x - 1][prey.y] > 0)
            return 1;
        return 0;
        break;
    case 'r':
        if (predator_map[self_id][prey.x][prey.y + 1] > 0)
            return 1;
        return 0;
        break;
    case 'd':
...
int predator_strategy(int self_id)
{
    ...
    //視界に入っていない場合の初期値は-10にセットし，-10である場合は未発見であると判断
    predator1.x = -10;
    predator1.y = -10;
    predator2.x = -10;
    predator2.y = -10;
